...
abstract class Scene extends Phaser.Scene {
  ...
  constructor(config: string | Phaser.Types.Scenes.SettingsConfig) {
    super(config)
    this.entityManager = new EntitiesManager()
    Scene.instance = this
  }
  ...
  debugEntities(entities: (Entity | string)[], position?: number[]): string {
    ...
    const msg = (
      entities.length ? entities : this.entityManager.getAllEntities()
    )
      .map((entityOrTag) => {
        ...
        const tag = (
          isTag
            ? entityOrTag
            : this.entityManager.getTagByEntity(entityOrTag as Entity)
        ) as string
        let msg = `--${tag || entity}--\n`
